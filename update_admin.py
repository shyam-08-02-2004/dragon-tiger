...
# 3. loadData
content = re.sub(
    r"const settingsStr = localStorage\.getItem\('dragonTigerAdminSettings'\) \|\| '\{\"forcedOutcome\":\"none\"\}';.*?\} catch \(e\) \{.*?setForcedOutcome\('none'\);\s*\}",
...
content = re.sub(
    r"const saveForcedOutcome = \(outcome: string\) => \{.*?localStorage\.setItem\('dragonTigerAdminSettings', JSON\.stringify\(settings\)\);\s*\};",
...
content = re.sub(
    r"<h3>Force Next Round Result<\/h3>.*?<\/div>",
...
content = re.sub(
    r"(<button \s*className=\"action-btn edit\" \s*onClick=\{\(\) => \{\s*setEditBalanceUser\(username\);\s*setNewBalance\(data\.balance\.toString\(\)\);\s*\}\}\s*title=\"Edit Balance\"\s*>\s*💰\s*<\/button>)",
    r"""\1
                              <button 
