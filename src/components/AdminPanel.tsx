              ...
              {!selectedSupportUser ? (
                ...
                <div style={{ flex: 1, display: 'flex', flexDirection: 'column' }}>
                  <div className="admin-chat-header-premium" style={{ position: 'sticky', top: 0, zIndex: 100 }}>
                    ...
                    <div className="chat-username" style={{ fontSize: '18px', color: '#D4AF37' }}>VIP Support</div>
                    <div style={{ width: '80px' }}></div>
                  </div>
                  ...
                  <div style={{ flex: 1, overflowY: 'auto', padding: '15px' }}>
                    {supportUsers.map(u => (
                      ...
                      >
                        <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
                          <div style={{ width: '40px', height: '40px', borderRadius: '50%', background: 'linear-gradient(135deg, rgba(212,175,55,0.2), rgba(212,175,55,0.05))', border: '1px solid rgba(212,175,55,0.3)', display: 'flex', justifyContent: 'center', alignItems: 'center', color: '#D4AF37' }}>
                            👤
                          </div>
                          <span style={{ fontWeight: 'bold', fontSize: '16px', color: '#fff' }}>{u.userId}</span>
                        </div>
                        {u.unreadCount > 0 && (
                          <span style={{ background: '#e74c3c', color: 'white', borderRadius: '20px', padding: '4px 10px', fontSize: '12px', fontWeight: 'bold', boxShadow: '0 0 10px rgba(231,76,60,0.5)' }}>
                            {u.unreadCount} New
                          </span>
                        )}
                      </div>
                    ))}
                    {supportUsers.length === 0 && <div style={{ padding: '40px 20px', color: '#888', textAlign: 'center', fontSize: '16px' }}>No active chats</div>}
                  </div>
                </div>
              ) : (
                /* CHAT VIEW */
                <div className="admin-chat-main">
                  <div className="admin-chat-header-premium" style={{ position: 'sticky', top: 0, zIndex: 100 }}>
                    ...
                    <div className="admin-chat-user-info">
                      <div className="chat-avatar" style={{ background: 'transparent', border: '1px solid #D4AF37', color: '#D4AF37' }}>
                        👤
                      </div>
                      <span className="chat-username" style={{ color: '#fff' }}>{selectedSupportUser}</span>
                    </div>
                          ...
                          {editingChatId === msg.id ? (
                            <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                              <input 
                                ...
                                style={{ width: '100%', padding: '8px', borderRadius: '6px', border: 'none', color: '#000', outline: 'none' }}
                                autoFocus
                              />
                              <div style={{ display: 'flex', gap: '8px', justifyContent: 'flex-end' }}>
                                <button onClick={() => setEditingChatId(null)} style={{ background: '#ccc', color: '#000', border: 'none', borderRadius: '6px', padding: '6px 12px', cursor: 'pointer', fontSize: '12px', fontWeight: 'bold' }}>Cancel</button>
                                <button onClick={() => handleAdminEditMessage(msg.id, editingChatText)} style={{ background: '#2ecc71', color: '#fff', border: 'none', borderRadius: '6px', padding: '6px 12px', cursor: 'pointer', fontSize: '12px', fontWeight: 'bold' }}>Save</button>
                              </div>
                            </div>
                          ) : (
                            <div>
                              {msg.imageUrl && (
                                <div className="admin-chat-media-container" onClick={() => setFullScreenMedia({ url: msg.imageUrl!, type: msg.mediaType === 'video' ? 'video' : 'image' })}>
                                  {msg.mediaType === 'video' ? (
                                    <video src={msg.imageUrl} className="admin-chat-video" style={{ width: '100%', borderRadius: '8px', display: 'block' }} />
                                  ) : msg.mediaType === 'pdf' ? (
                                    <embed src={msg.imageUrl} type="application/pdf" className="admin-chat-pdf" style={{ width: '100%', height: '200px', borderRadius: '8px' }} />
                                  ) : (
                                    <>
                                      <img src={msg.imageUrl} alt="attachment" className="admin-chat-image" style={{ borderRadius: '8px' }} />
                                      <div className="media-overlay-hint">Tap to view Full Screen</div>
                                    </>
                                  )}
                                </div>
                              )}
                              {msg.message && <div className="admin-chat-text" style={{ fontSize: '15px', lineHeight: '1.4' }}>{msg.message}</div>}
